...
### Injected Issues
❌ **No validation for password length or complexity.**  
❌ **First Name allows only letters but allows single-character names.**  
❌ **Surname validation allows special characters except for numbers.**  
❌ **Country and City fields have no character validation (e.g., numbers or special symbols allowed).**  
❌ **Registration form does not enforce a minimum password length or require special characters.**  
❌ **There is no confirmation password field to prevent accidental mistakes.**  
❌ **No validation preventing spaces-only inputs for required fields.**  
...
### Injected Issues
❌ **User is not locked after the 3rd unsuccessful login attempt.**
...
### Injected Issues
❌ **The password field is missing from the profile update form.**  
❌ **The surname field allows single-character input, violating the minimum length requirement.**  
❌ **There is no validation to prevent special characters in the First Name and Surname fields.**  
...
### Injected Issues
❌ **SQL query logic is incorrect: `WHERE` clause does not correctly group country and city conditions with `OR`.**  
❌ **SQL query is vulnerable to SQL injection due to string formatting with `%s` placeholders.**  
❌ **Pagination is missing, leading to potential performance issues when retrieving large datasets.**  
❌ **Duplicate user entries are not handled correctly when a user has multiple matching skills.**  